...
基于阿里云 **Qwen-Omni-Turbo** 多模态模型的语音对话智能体。

## 架构

```
音频输入 → 声学 VAD → Qwen-Omni-Turbo (多模态 LLM) → TTS → 音频输出
```

**✨ 优势：**
- **无需 ASR** - Qwen-Omni-Turbo 直接理解音频
- **低延迟** - 减少中间环节
- **更自然** - 模型直接处理语音韵律、语气
...
| **LLM** | Qwen-Omni-Turbo | 多模态大模型，直接处理音频 + 文本 |
...
│   ├── llm.py                # Qwen-Omni-Turbo LLM (多模态)
│   ├── tts.py                # TTS 封装
│   └── __init__.py
├── examples/
│   ├── chat_demo.py          # 文本对话
...
- **阿里云 DashScope**
  - Qwen-Omni-Turbo (多模态 LLM) - 音频 + 文本理解
  - SpeechSynthesizer (TTS) - 语音合成

## 注意事项

1. **Qwen-Omni-Turbo** 支持直接音频输入，音频需要转为 data URI 格式
